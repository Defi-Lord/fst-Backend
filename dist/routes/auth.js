...
router.post("/nonce", async (req, res) => {
    try {
        ...
        await prisma_js_1.prisma.nonce.upsert({
...
router.post("/verify", async (req, res) => {
    try {
        ...
        const stored = await prisma_js_1.prisma.nonce.findUnique({ where: { address: walletAddress } });
        ...
        const pubkeyBytes = bs58_1.default.decode(walletAddress);
        const valid = tweetnacl_1.default.sign.detached.verify(msgBytes, sigBytes, pubkeyBytes);
        if (!valid)
            return res.status(401).json({ success: false, error: "invalid_signature" });
        let wallet = await prisma_js_1.prisma.wallet.findUnique({ where: { address: walletAddress } });
        if (!wallet)
            wallet = await prisma_js_1.prisma.wallet.create({ data: { address: walletAddress } });
        ...
        await prisma_js_1.prisma.nonce.deleteMany({ where: { address: walletAddress } });
