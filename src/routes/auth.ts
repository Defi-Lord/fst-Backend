...
router.post("/nonce", async (req: Request, res: Response) => {
  try {
    ...
    // upsert nonce for the wallet address
...
router.post("/verify", async (req: Request, res: Response) => {
  try {
    ...
    // recreate the exact message that was stored
    ...
    try {
      ...
      // fallback to base58 if client sent bs58
    ...
    // determine role (ADMIN detection via env list)
    const role = isAdminWallet(walletAddress) ? "ADMIN" : "USER";

    // IMPORTANT: issue token where userId == wallet address (so middleware & frontend match)
    const payload = { userId: wallet.address, role, typ: "access" };
    const token = jwt.sign(payload, JWT_SECRET, { expiresIn: "30d" });

    // remove used nonces
    await prisma.nonce.deleteMany({ where: { address: walletAddress } });

    return res.json({ success: true, token, walletId: wallet.id, role });
  } catch (err) {
