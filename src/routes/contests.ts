...
router.post('/', requireAdmin, async (req, res) => {
  ...
  if (!slug || !title || !realm || !playersLimit) return res.status(400).json({ error: 'slug, title, realm, playersLimit required' })

  const data: any = { slug, title, realm, playersLimit, transferpool: transferpool ?? 0 }
